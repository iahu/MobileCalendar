...
extend(MobileCalendar.prototype, {
    ...
    bindUI: function () {
        ...
        .on('touchstart', '.closer', function(event) {
            event.preventDefault();
            __self.remove.call(__self);
        });
    ...
    swapMonth: function (event) {
        ...
        if (event.type === 'moveleft') {
            ...
            newYear = newYear + Math.floor(((this.currentMonth - 1) || -1)/13);
            clsCur = 'slideOutRight';
            clsNew = 'slideInLeft';
        }
        this.currentYear = newYear;
        this.currentMonth = newMonth;
        html = this.renderMonth( new Date(this.currentYear, this.currentMonth - 1,1,0,0,0) );
